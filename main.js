window.addEventListener("load", () => {
  ...
  function isTaskDuplicate(taskText) {
    ...
    return tasks.some((t) => t.text === taskText);
  }

  function saveTasksToLocalStorage() {
    const tasks = [];
    const taskItems = list_el.querySelectorAll(".task");

    taskItems.forEach((taskItem) => {
      const taskText = taskItem.querySelector(".text").value;
  ...
  function createTaskElement(taskText, isCompleted) {
    ...
    if (isCompleted) task_el.classList.add("completed");
